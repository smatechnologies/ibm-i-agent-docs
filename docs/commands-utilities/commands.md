...
Most commands work only from within a job started or tracked by OpCon.

LFEEDBACK: Send LSAM Feedback character string to OpCon
...
- Refer to the IBM i LSAM installation instructions. Also refer to the topic of Managing Multiple Environments in Installing Multiple Environments and LSAM Environment Management. This
command is not available and is not used outside of an installation from media.
...
- This command also represents the environment selection function that can be used as a first
step for either the STRSMA or SMASETLIBL command.
